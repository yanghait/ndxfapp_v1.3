package com.ynzhxf.nd.firecontrolapp.network.observer;

import com.google.gson.Gson;
import com.trello.rxlifecycle2.LifecycleProvider;
import com.trello.rxlifecycle2.android.ActivityEvent;
import com.trello.rxlifecycle2.android.FragmentEvent;
import com.ynzhxf.nd.firecontrolapp.network.function.HttpResultFunction;
import com.ynzhxf.nd.firecontrolapp.network.function.ServerResultFunction;
import com.ynzhxf.nd.firecontrolapp.network.function.ObjectServerResultFunction;
...
public class HttpRxObservable {
    ...
    public static Observable getObjectObservable(Observable apiObservable, LifecycleProvider<ActivityEvent> lifecycle, ActivityEvent event) {
        // showLog(request);
        Observable observable;
        if (lifecycle != null) {
            //手动管理移除监听生命周期.eg:ActivityEvent.STOP
            observable = apiObservable
                    .map(new ObjectServerResultFunction())
                    .compose(lifecycle.bindUntilEvent(event))//需要在这个位置添加
                    .onErrorResumeNext(new HttpResultFunction<>())
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread());

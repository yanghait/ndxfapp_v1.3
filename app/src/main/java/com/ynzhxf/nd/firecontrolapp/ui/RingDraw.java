...
public class RingDraw extends View {
    ...
    public RingDraw(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        pbac = new Paint();
        ...
        endColor = Color.parseColor("#ffcd02");
        statrColor = Color.parseColor("#ff3155");
    ...
    protected void onDraw(Canvas canvas) {
        ...
        synchronized (this){
            ...
            canvas.drawText(nowCount+"", width/2,height/2 + circleWidth/2, paint);
            pbac.setColor(Color.parseColor("#ffffff"));
            pbac.setShader(null);
            canvas.drawCircle(width/2 , height/2 ,height/2-circleWidth , pbac);

            if(canLoad){
                float x = 0+circleWidth;
                float y = 0+circleWidth;
                RectF oval = new RectF( x, y,width -circleWidth, height - circleWidth);
                pbac.setStyle(Paint.Style.STROKE);
                linearGradient = new  LinearGradient(0, 0, width, height, endColor,statrColor , Shader.TileMode.MIRROR);
